...
export const useFormValidation = (form: CreateDeliveryCompanyInput) => {
  ...
  const validateNameEn = useCallback(
    (value: string): string | null => {
      if (!value || value.trim() === "") {
        return dict.delivery_companies_page.validation.nameEnRequired;
      }
      if (value.trim().length < NAME_MIN_LENGTH) {
        return dict.delivery_companies_page.validation.nameEnMinLength;
      }
  ...
  const validateNameAr = useCallback(
    (value: string): string | null => {
      if (!value || value.trim() === "") {
        return dict.delivery_companies_page.validation.nameArRequired;
      }
      if (value.trim().length < NAME_MIN_LENGTH) {
        return dict.delivery_companies_page.validation.nameArMinLength;
      }
      if (value.trim().length > NAME_MAX_LENGTH) {
  ...
  const validateStatus = useCallback(
    (value: DeliveryCompanyStatus): string | null => {
      if (!value) {
        return dict.delivery_companies_page.validation.statusRequired;
      }
      if (!Object.values(DeliveryCompanyStatus).includes(value)) {
        return dict.delivery_companies_page.validation.statusInvalid;
      }
      return null;
    },
    [dict],
  ...
  const validateField = useCallback(
    (
      field: keyof CreateDeliveryCompanyInput,
      value: string | DeliveryCompanyStatus,
    ) => {
      let error = "";
      ...
      switch (field) {
        ...
        case "nameAr":
          ...
          break;
        case "status":
          error = validateStatus(value as DeliveryCompanyStatus) || "";
          break;
      }
